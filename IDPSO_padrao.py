...
def enviarObservadoras(np, t, tmax):
    ...
    for fonte in range(np):    
        fi = calcularFi(particulas[fonte], PBEST[fonte], PBEST[indiceGBEST])
        #if fi > 3.:
        #    fi = 3.
        if fi > -1:
            w[fonte] = ((wmax - wmin)/(1 + exp(fi*(t-((1+log(fi))*tmax)/mi)))) + wmin
            c1[fonte] = c1[fonte]*(fi**-1)
            c2[fonte] = c2[fonte]*fi          
        
        r1 = random.rand()
        r2 = random.rand()
        
        velocidades[fonte] = ajustarVelocidade(w[fonte]*velocidades[fonte] + c1[fonte]*r1*(PBEST[fonte] - particulas[fonte]) + c2[fonte]*r2*(PBEST[indiceGBEST] - particulas[fonte]))
