...
def calcularCriterioJ(particula, pesos, classesParticula):
    ...
    sumDistanciasProtsMembros = sum((distancias.min(1) / sum(around(particula[0,:pa]))) * (classesTreinamento == classesParticula[argmin(distancias,1)]))
    ...
    criterio = sumDistanciasProtsMembros / len(classesTreinamento)#/ sum(distanciasProtCentros)  
...
def calcularPesosPrototipos(prototipos, classesPrototipos, variaveisConsideradas, pesos = []):
    ...
    particao = argmin(distancias,1)
    #distanciasCorretas = distancias.min(1) * (classesTreinamento == classesPrototipos[particao])
...
if __name__ == "__main__":
    ...
    print "vabc", alfa, beta, "tradicionalhard/100", prots
    print nome
